...
class PoolNameFormatterTest extends PHPUnit_Framework_TestCase {
   ...
   public function test_format_with_default() {
      $this->assertEquals( 'Pool C7', PoolNameFormatter::format_with_default( 7, 3 ) );
      $this->assertEquals( 'Pool 7', PoolNameFormatter::format_with_default( 7 ) );
   }

   public function test_format() {
      static $FMT = '%t(uc)-%L, %P T%t(num)/P%p(num) : %p(uc)%%';
      $this->assertEquals( 'C-League, Pool T3/P7 : G%', $this->format( $FMT, 7, 3 ) );
      $this->assertEquals( '-League, Pool T/P7 : G%', $this->format( $FMT, 7, -1 ) );
      $this->assertEquals( '29-League, Pool T29/P28 : 28%', $this->format( $FMT, 28, 29 ) );
   }
   ...
   private function format( $format, $pool, $tier )
   {
      $pn = new PoolNameFormatter($format);
      return $pn->format($pool, $tier);
   }
