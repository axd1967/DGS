...
require( "include/board_test.php" );
...
while( $row = mysql_fetch_array($moves_result) )
{
    ...
    if( $i % $moves_per_row == 0 )
      {
        ?></tr>
<tr align=center><td><?php echo $i;?>-<?php echo ($i + $moves_per_row - 1);         printf (" %0.5f", getmicrotime() - $time );
?></td>
<?php
      }
    ...
    if( $i == $move )
      {
      ?><td  class="r"><?php echo $c;?></td>
<?php
      }
    else if( $s == BLACK )
      {
        ?><td><a class="b" href=game.php?gid=<?php echo $t;?></A></td>
<?php
      }
    else
      {
