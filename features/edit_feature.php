   ...
   if ( @$_REQUEST['feature_save'] || $is_preview )
   {
      ...
      if ( !$is_preview && is_null($errormsg) )
      {
         ...
         {//HOT-section to update feature
            ...
            // re-calc new-feature-count for all users (as it may differ if users already voted on feature)
            if ( $new_status == FEATSTAT_VOTE && ( $fid == 0 || $old_status != $new_status ) )
               Feature::update_count_feature_new( "edit_feature.upd_feature.to_new($fid)", 0, null );
            elseif ( $old_status == FEATSTAT_VOTE && $fid > 0 && $old_status != $new_status )
               Feature::update_count_feature_new( "edit_feature.upd_feature.from_new($fid)", 0, null );
         }
         ta_end();
