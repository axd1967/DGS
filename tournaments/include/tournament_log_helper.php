...
class TournamentLogHelper
{
   ...
   public static function log_tournament_game_end( $tid, $tlog_type, $gid, $edits, $old_score, $new_score,
         $old_flags, $new_flags, $old_status, $new_status )
   {
      $msg = array();
   ...
   private static function build_diff_tournament_participant( $old_tp, $new_tp )
   {
      ...
      if ( $old_tp->DirectorMessage != $new_tp->DirectorMessage )
         $msg[] = sprintf(self::$DIFF_FMT, 'DirMsg', $old_tp->DirectorMessage, $new_tp->DirectorMessage );
   ...
   public static function log_withdraw_user_from_tournament_ladder( $tid, $tlog_type, $tladder, $count_run_games )
   {
      $tlog = new Tournamentlog( 0, $tid, 0, 0, $tlog_type, 'TL_User', TLOG_ACT_CHANGE, $tladder->uid,
         "Ladder-Withdrawal initiated: run-tgames=[$count_run_games] for User: " . $tladder->build_log_string() );
      $tlog->insert();
   }

   public static function log_start_withdraw_user_from_tournament_ladder( $tid, $tlog_type, $tladder, $cause )
   {
      $tlog = new Tournamentlog( 0, $tid, 0, 0, $tlog_type, 'TL_User', TLOG_ACT_CHANGE, $tladder->uid,
         "Ladder-Withdrawal started ($cause): for User: " . $tladder->build_log_string() );
      $tlog->insert();
   }
   ...
   {
      $tlog = new Tournamentlog( 0, $tid, 0, 0, $tlog_type, 'TL_User', TLOG_ACT_CHANGE, $tladder->uid,
         "Ladder-Withdrawal revoked: for User: " . $tladder->build_log_string() );
      $tlog->insert();
   }
   ...
   public static function log_set_tournament_pool_ranks( $tid, $tlog_type, $ref_title, $tpool_ids, $rank, $fix_rank, $upd_count )
   {
      $tlog = new Tournamentlog( 0, $tid, 0, 0, $tlog_type, 'TPOOL_Rank', TLOG_ACT_SET, 0,
         sprintf('Set ranks[%s] (fix_rank=%s) for pool-IDs [%s]: rank=%s -> [%s] (%s entries updated)',
            $ref_title, ( $fix_rank ? 1 : 0 ),
            ( is_array($tpool_ids) ? implode(', ', $tpool_ids) : $tpool_ids ),
            $rank, ($upd_count ? 'OK' : 'FAILED'), $upd_count ));
      $tlog->insert();
   }
